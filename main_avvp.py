...
def train(args, model, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        audio, video, video_st, target = sample['audio'], sample['video_s'], sample['video_st'], sample['label'].type(torch.FloatTensor)
...
def eval(model, val_loader, set):
    ...
    df_a = pd.read_csv("hacker/AVVP-ECCV20-master/data/AVVP_eval_audio.csv", header=0, sep='\t')
    df_v = pd.read_csv("hacker/AVVP-ECCV20-master/data/AVVP_eval_visual.csv", header=0, sep='\t')
...
def main():
    ...
    parser.add_argument(
        ...
        help="video dir")
    parser.add_argument(
        "--label_train", type=str, default="hacker/AVVP-ECCV20-master/data/AVVP_train.csv", help="weak train csv file")
    parser.add_argument(
        "--label_val", type=str, default="hacker/AVVP-ECCV20-master/data/AVVP_val_pd.csv", help="weak val csv file")
    parser.add_argument(
        "--label_test", type=str, default="hacker/AVVP-ECCV20-master/data/AVVP_test_pd.csv", help="weak test csv file")
    ...
    parser.add_argument(
        "--model_save_dir", type=str, default='hacker/AVVP-ECCV20-master/models/', help="model save dir")
    parser.add_argument(
    ...
    parser.add_argument(
    '--pool_mode', type=str, default='att', help='mode of pooling : max, mean, att')
    ...
    if args.mode == 'train':
        ...
        model.load_state_dict(torch.load(args.model_save_dir + args.checkpoint + ".pt", map_location=torch.device('cpu')))
